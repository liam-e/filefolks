...
import { imagesToPdf } from "@/lib/processors/pdf";
import { downloadBlob, formatFileSize } from "@/lib/utils/file";
import { IMAGE_ACCEPT } from "@/lib/processors/image";
...
export function ImageToPdfTool() {
  ...
  const buttonLabel =
    status === "converting"
      ? t("converting")
      : files.length === 1
        ? t("convertButton", { count: 1 })
        : t("convertButtonPlural", { count: files.length });
